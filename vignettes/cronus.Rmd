...
cronus is an R package that focuses on database creation. It can be used to download and process agricultural, environmental and satellite data from various sources. This vignette contains general information on the package.

## Understanding the package in 5 minutes
...
In order to avoid specifying the database directory in every single function call, a variable named `path_demeter` can be added to the environment variables, stored in the user or project .Renviron file. This allows the user to specify only once the database directory, which will load in the beginning of every R session. Function `set_path_demeter()` can be used to specify or change the path.

```{r}
set_path_demeter(path = getwd(), level = "user")
```
