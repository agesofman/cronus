...
#' @param pre character. A vector of dates to be used as an extra smoothing window before the dates of interest.
#' @param post character. A vector of dates to be used as an extra smoothing window after the dates of interest.
...
setMethod("smoothout",
          ...
          definition = function(x, variable, smoother = "wt1", pre = NULL, post = NULL) {
  ...
  dir_smooth <- create_db(dir, region, product = product, variable = paste0(variable, "_smooth_", smoother))
  ...
  temp <- terra::app(ras_var, smoother, filename = path_temp, overwrite = TRUE, wopt = list(names = toi$name))
  ...
  for (i in 1:length(date)) {
    pb$tick()
    terra::writeRaster(temp[[toi$name[i]]], filename = path_smooth[i], overwrite = TRUE)
  }
